...
INSTALLED_VER=$(python3 -c "import importlib.metadata; print(importlib.metadata.version('federaide-dev-nightly'))" 2>/dev/null || \
                python3 -c "import pkg_resources; print(pkg_resources.get_distribution('federaide-dev-nightly').version)" 2>/dev/null || \
                uv tool list 2>/dev/null | grep -E '^federaide-dev-nightly ' | awk '{print $2}' | tr -d 'v' || echo "")
...
EOF
    ...
    if [ "$DOWNLOAD_SUCCESS" = true ]; then
        ...
        uv tool install --force --upgrade --refresh --python 3.13 --find-links "$TYRES_DIR" "federaide-dev-nightly[all]"
    else
        echo "[!] Pre-compiled wheels not found."
        echo "[!] Falling back to basic upgrade to prevent compilation hangs."
        uv tool install --force --upgrade --refresh --python 3.13 federaide-dev-nightly
    fi
    rm -rf "$TYRES_DIR"
    ...
    echo "[*] Upgrading FEDERaiDE on Python 3.13..."
    uv tool install --force --upgrade --refresh --python 3.13 "federaide-dev-nightly[all]"
fi
