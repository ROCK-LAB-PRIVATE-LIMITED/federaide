...
if ($toolList -match 'federaide-dev-nightly\s+v([\d\.]+)') {
    $installedVer = $Matches[1]
}

if (-not $installedVer) {
    try {
        $installedVer = python -c "import importlib.metadata; print(importlib.metadata.version('federaide-dev-nightly'))" 2>$null
...
function Get-PyPiVersion {
    try {
        ...
        $url = "https://pypi.org/pypi/federaide-dev-nightly/json?cb=$cb"
...
uv tool install --force --upgrade --refresh --python 3.13 "federaide-dev-nightly[audio,ide,vision]"
