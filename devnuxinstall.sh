...
install_federaide-dev-nightly() {
    ...
    if is_musl; then
        echo "[*] musl-based C library detected (e.g. Alpine). Targeting federaide-dev-nightly[ide,vision,pdf] (omitting audio/onnxruntime)..."
        TARGET_EXTRAS="federaide-dev-nightly[ide,vision,pdf]"
    else
        TARGET_EXTRAS="federaide-dev-nightly[all]"
    fi
...
setup_path_and_symlinks() {
    ...
    if [ "$(id -u)" -eq 0 ] || command -v sudo >/dev/null 2>&1 || command -v doas >/dev/null 2>&1; then
        ...
        TMP_PROFILE="${TMPDIR:-/tmp}/federaide-dev-nightly_profile.sh"
        cat << 'EOF' > "$TMP_PROFILE"
...
EOF
        run_root cp "$TMP_PROFILE" /etc/profile.d/federaide-dev-nightly.sh 2>/dev/null || true
        run_root chmod 644 /etc/profile.d/federaide-dev-nightly.sh 2>/dev/null || true
        rm -f "$TMP_PROFILE"
    fi
    ...
    for bin_name in "federaide-dev-nightly" "federate"; do
        TARGET_BIN=""
